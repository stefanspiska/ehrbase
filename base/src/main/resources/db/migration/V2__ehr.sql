...
COMMENT
ON TABLE  ehr.system IS 'system table for reference';
...
COMMENT
ON TABLE  ehr.territory IS 'ISO 3166-1 countries codeset';
...
COMMENT
ON TABLE  ehr.language IS 'ISO 639-1 language codeset';
...
COMMENT
ON TABLE  ehr.terminology_provider IS 'openEHR identified terminology provider';
...
COMMENT
ON TABLE  ehr.concept IS 'openEHR common concepts (e.g. terminology) used in the system';
...
COMMENT
ON TABLE ehr.identifier IS 'specifies an identifier for a party identified, more than one identifier is possible';
...
COMMENT
ON TABLE ehr.access IS 'defines the modality for accessing an com.ethercis.ehr (security strategy implementation)';
--
...
COMMENT
ON TABLE ehr.ehr IS 'EHR itself';
...
(
    ...
    is_queryable    boolean default true,
    is_modifiable   boolean default true,
    party           UUID      not null, -- subject (e.g. patient)
    other_details   JSONB,
    sys_transaction TIMESTAMP NOT NULL,
    sys_period      tstzrange NOT NULL  -- temporal table
);
...
COMMENT
ON TABLE ehr.status IS 'specifies an ehr modality and ownership (patient)';
...
-- change history table
CREATE TABLE ehr.contribution_history
(
    id                  UUID,
    ehr_id              UUID,
    contribution_type   ehr.contribution_data_type, -- specifies the type of data it contains
...
COMMENT
ON TABLE ehr.contribution IS 'Contribution table, compositions reference this table';
...
(
    ...
    ehr_id          UUID,
--    version UUID references ehr.versioned(id),
    in_contribution UUID,                 -- in contribution version
    active          boolean default true, -- true if this composition is still valid (e.g. not replaced yet)
    is_persistent   boolean default true,
    language        varchar(5),           -- pointer to the language codeset. Indicates what broad category this Composition is belogs to, e.g. �persistent� - of longitudinal validity, �event�, �process� etc.
    territory       int,                  -- Name of territory in which this Composition was written. Coded fromBinder openEHR �countries� code set, which is an expression of the ISO 3166 standard.
    composer        UUID      not null,   -- points to the PARTY_PROXY who has created the composition
    sys_transaction TIMESTAMP NOT NULL,
    sys_period      tstzrange NOT NULL    -- temporal table
    -- item UUID not null, -- point to the first section in composition
);
...
COMMENT
ON TABLE ehr.composition IS 'Composition table';
...
(
    ...
    setting         ehr.dv_coded_text,                       -- codeset setting, see ehr_im section 5
...
(
    id              UUID,
    composition_id  UUID,              -- belong to composition
    start_time      TIMESTAMP not null,
    start_time_tzid TEXT,              -- time zone id: format GMT +/- hh:mm
    end_time        TIMESTAMP null,
    end_time_tzid   TEXT,              -- time zone id: format GMT +/- hh:mm
    facility        UUID,              -- points to a party identified
    location        TEXT,
    other_context   JSONB,             -- supports a cluster for other context definition
    setting         ehr.dv_coded_text,              -- codeset setting, see ehr_im section 5
--	program UUID references ehr.program(id), -- the program defined for this context (only in full ddl version)
...
COMMENT
ON TABLE ehr.event_context IS 'defines the context of an event (time, who, where... see openEHR IM 5.2';
...
COMMENT
ON TABLE ehr.participation IS 'define a participating party for an event f.ex.';
...
(
    ...
    category        ehr.dv_coded_text ,                   -- used to specify the type of content: Evaluation, Instruction, Observation, Action with different languages
...
COMMENT
ON TABLE ehr.entry IS 'this table hold the actual archetyped data values (fromBinder a template)';
...
CREATE
OR REPLACE VIEW ehr.comp_expand AS
