...
(
    ...
    system_id           UUID references ehr.system (id) NOT NULL,
    committer           UUID references ehr.party_identified (id) NOT NULL,
    time_committed      timestamp        default NOW(),
    time_committed_tzid TEXT NOT NULL,              -- timezone id
...
CREATE TABLE ehr.audit_details_history
(
    id                  UUID,
    system_id           UUID,
    committer           UUID ,
...
COLUMN time_committed,
    DROP
COLUMN time_committed_tzid, -- timezone id
    DROP
COLUMN change_type,
    DROP
COLUMN description;
...
ALTER TABLE ehr.contribution_history
    ADD COLUMN has_audit UUID references ehr.audit_details (id) ON DELETE CASCADE;
-- has this audit_details instance
